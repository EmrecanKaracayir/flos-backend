...
export enum SignupQueries {
  DOES_ORGANIZER_EXIST_$UNAME = `SELECT EXISTS (SELECT * FROM "Organizer" WHERE "username" = $1) AS "exists"`,
  DOES_ORGANIZER_EXIST_$EMAIL = `SELECT EXISTS (SELECT * FROM "Organizer" WHERE "email" = $1) AS "exists"`,
  CREATE_ORGANIZER_$UNAME_$PSWRD_$EMAIL = `INSERT INTO "Organizer" ("username", "password", "email") VALUES ($1, $2, $3) RETURNING *`,
  DOES_PARTICIPANT_EXIST_$UNAME = `SELECT EXISTS (SELECT * FROM "Participant" WHERE "username" = $1) AS "exists"`,
  DOES_PARTICIPANT_EXIST_$EMAIL = `SELECT EXISTS (SELECT * FROM "Participant" WHERE "email" = $1) AS "exists"`,
